...
setwd("~/Dropbox/Rafael restauracao/Insect_plant restoration")
...
###################### --------------------------- Diversity 2017
setwd("~/Dropbox/Rafael restauracao/Data/Matrizes 2017")
...
d2017<-lapply(myfiles, read.csv, row.names=1, header = T)# read data from files, quickly
head(d2017[[1]]) # validate file reading
names(d2017) <- myfiles # assign names to list item
...
#ordering dataframes so that the order of species list matches the order of species in graph g
guild <- guilds[order(match(guilds$species, V(g17)$name)),] 
...
################### diversity plots
###d2017
results_c2017<-sapply (d2017, FUN=networklevel, index="connectance") 
###d2018
...
write.csv(k_2017_p, "k_insects_2018.csv")
